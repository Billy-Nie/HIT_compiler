...
def token_recognition(token, digitDFA, charDFA, stringDFA, commentDFA, o_H_DFA, line_number,
    ...
    if isAlpha(token[0]):
        if token in keywords:
            ...
            token_l.append(token + "\tIDN\t" + token + "\t" + str(line_number))
    ...
    if isDigit(token[0]):
        ...
        if digitDFA_result:
            if isfloat:
                ...
                token_l.append(token + "\tFLOAT\t" + token + "\t" + str(line_number))
            else:
                # print(token + "\t整形常量\t" + token + "\t" + str(line_number))
                #if token + "\t整形常量\t" + token + "\t" + str(line_number) not in token_l:
                token_l.append(token + "\tINT10\t" + token + "\t" + str(line_number))
        ...
        if o_H_DFA_result:
            if is_octal:
                ...
                token_l.append(token + "\tINT8\t" + token+ "\t" + str(line_number))
            else:
                # print(token + "\t<十六进制常量\t" + token + ">\t" + str(line_number))
                #if token + "\t十六进制常量\t" + token + "\t" + str(line_number) not in token_l:
                token_l.append(token + "\tINT16\t" + token + "\t" + str(line_number))
    ...
    if isChar(token[0]):
        ...
        if charDFA_result:
            ...
            token_l.append(token + "\tCHAR\t" + token + "\t" + str(line_number))
        else:
    ...
    if isString(token[0]):
        ...
        if stringDFA_result:
            ...
            token_l.append(token + "\tSTR\t" + token + "\t" + str(line_number))
        else:
...
def Latex_analyse_main(file_path, digitDFA = digitDFA, charDFA = charDFA, stringDFA = stringDFA, commentDFA = commentDFA, o_H_DFA = o_H_DFA):
    character_table = []  # 符号表
    token_l = []
    error_l = []
    #print(error_l)
...
# _,_,token_l = Latex_analyse_main("/Users/billy/Junior/编译系统/实验/Lab2_Grammar_Analyse/file/test", digitDFA, charDFA, stringDFA, commentDFA, o_H_DFA)
